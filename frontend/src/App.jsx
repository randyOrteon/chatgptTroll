      ...
      <header className="flex justify-between items-center p-2 sm:p-4 border-b border-gray-200">
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div className="flex gap-1 items-center">
            <span className="font-semibold text-lg sm:text-xl text-zinc-700">
              ChatGPT{" "}
            </span>
            <IoIosArrowDown />
          </div>
        </div>
        <h1 className="text-lg sm:text-xl font-semibold hidden sm:flex gap-2 items-center">
          New Chat{" "}
        </h1>
        <div className="flex items-center gap-2  sm:gap-6">
          <motion.button
            ...
            onClick={shareChat}
          >
            <img src={share} alt="share" className="h-3 sm:h-5" />{" "}
            <span className="hidden sm:inline">Share</span>
          </motion.button>
          <div className="p-1 sm:p-2 rounded-full bg-slate-200 group cursor-pointer">
            <User2Icon className="w-4 h-4 sm:w-6 sm:h-6" />
          </div>
        </div>
      </header>
        ...
        <AnimatePresence>
          {chat.map((msg, idx) => {
            const isLastResponderMessage =
              msg.role === "responder" && idx === chat.length - 1;
    ...
    const handleQuestion = ({ roomId, msg }) => {
      ...
      setActiveRooms((prevRooms) => {
        const updatedRooms = prevRooms.map((room) =>
          room.id === roomId ? { ...room, latestMessage: msg } : room
        );
        return updatedRooms;
      });
    };
              ...
              >
                <div className="flex items-center justify-between w-full sm:w-11/12 mx-auto">
                  <div className="flex items-center space-x-2 sm:space-x-3">
                    <MessageSquare
                      size={20}
                      className="text-[#6c71ff] sm:w-6 sm:h-6"
                    />
                    <div>
